    ...
    execute: function () {
      ...
      var YXCollectionView = exports('YXCollectionView', (_dec = ccclass('YXCollectionView'), _dec2 = disallowMultiple(true), _dec3 = help(""), _dec4 = property({
        ...
        _proto4.applyLayoutAttributes = function applyLayoutAttributes(node, attributes, bringToFront) {
          ...
          pos.x = -(this.layout.contentSize.width - transform.width) * 0.5 + attributes.frame.x;
          pos.y = +(this.layout.contentSize.height - transform.height) * 0.5 - attributes.frame.y;
          var matrix = math.Mat4.IDENTITY.clone();

          if (attributes.offset) {
            pos.add(attributes.offset);
          }

          math.Mat4.transform(matrix, matrix, pos);

          if (attributes.scale) {
            math.Mat4.scale(matrix, matrix, attributes.scale);
          }
